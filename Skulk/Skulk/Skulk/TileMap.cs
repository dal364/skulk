        ...
        public TileMap(int lvl, int map)
        {
            ...
            for (int y = 0; y < MapHeight; y++)
            {
                for (int x = 0; x < MapWidth; x++)
                {
                    ...
                    if (Convert.ToInt32(mapLineObstacle.Split(',')[x]) == 238)
                        this.mapCell[x, y].AddObject("hole");


                    if (Convert.ToInt32(mapLineObstacle.Split(',')[x]) != 0 && Convert.ToInt32(mapLineObstacle.Split(',')[x]) != 238)//integer corresponding to walls or boxes,etc
                    {
